...
- **max_parse_failure_retries**: New config option (default: `3`) limits retries when response parsing or capture fails (e.g. malformed JSON, length mismatch). When exceeded, the run raises a `RuntimeError` instead of retrying indefinitely. Override with `max_parse_failure_retries=5` on the CLI or in `conf/config.yaml`.
